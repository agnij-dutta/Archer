...
from features.config import BirdseyeModeEnum, ArcherConfig
from features.const import MODEL_CACHE_DIR
from features.detectors import DetectorTypeEnum
from features.plus import PlusApi
from features.util.builtin import deep_merge, load_config_with_no_duplicates
...
class TestConfig(unittest.TestCase):
    ...
    def test_config_class(self):
        Archer_config = ArcherConfig(**self.minimal)
        assert self.minimal == Archer_config.dict(exclude_unset=True)

        runtime_config = Archer_config.runtime_config()
    ...
    def test_detector_custom_model_path(self):
        ...
        Archer_config = ArcherConfig(**(deep_merge(config, self.minimal)))
        runtime_config = Archer_config.runtime_config()
    ...
    def test_invalid_mqtt_config(self):
        ...
        self.assertRaises(ValidationError, lambda: ArcherConfig(**config))
    ...
    def test_inherit_tracked_objects(self):
        ...
        Archer_config = ArcherConfig(**config)
        assert config == Archer_config.dict(exclude_unset=True)
    ...
    def test_plus_labelmap(self):
        ...
        runtime_config = Archer_config.runtime_config(PlusApi())
    ...
    def test_fails_on_missing_role(self):
        ...
        Archer_config = ArcherConfig(**config)
        self.assertRaises(ValueError, lambda: Archer_config.runtime_config())
    ...
    def test_fails_on_bad_camera_name(self):
        ...
        self.assertRaises(
            ValidationError, lambda: Archer_config.runtime_config().cameras
        )
    ...
    def test_fails_on_bad_segment_time(self):
        ...
        self.assertRaises(
            ValueError,
            lambda: Archer_config.runtime_config().ffmpeg.output_args.record,
        )
    ...
    def test_fails_zone_defines_untracked_object(self):
        ...
        self.assertRaises(ValueError, lambda: Archer_config.runtime_config().cameras)
    ...
    def test_fails_invalid_movement_weights(self):
        ...
        self.assertRaises(ValueError, lambda: ArcherConfig(**config))
