...
from features.const import BASE_DIR, BIRDSEYE_PIPE
...
class BirdsEyeFrameManager:
    def __init__(
        ...
        # initialize the frame as black and with the Archer logo
        ...
        if birdseye_logo is None:
            logo_files = glob.glob("/opt/Archer/Archer/images/birdseye.png")
        ...
        if birdseye_logo is not None:
            ...
            logger.warning("Unable to read Archer logo")
...
def output_frames(config: ArcherConfig, video_output_queue):
    threading.current_thread().name = "output"
    setproctitle("Archer.output")
