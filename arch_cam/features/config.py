...
from features.detectors import DetectorConfig, ModelConfig
from features.detectors.detector_config import BaseDetectorConfig
from features.ffmpeg_presets import (
...
from features.plus import PlusApi
from features.util.builtin import (
    deep_merge,
    escape_special_characters,
    get_ffmpeg_arg_list,
    load_config_with_no_duplicates,
)
from features.util.image import create_mask
from features.util.services import get_video_properties
...
Archer_ENV_VARS = {k: v for k, v in os.environ.items() if k.startswith("Archer_")}
...
class ArcherBaseModel(BaseModel):
    class Config:
        extra = Extra.forbid
...
class UIConfig(ArcherBaseModel):
...
class StatsConfig(ArcherBaseModel):
...
class TelemetryConfig(ArcherBaseModel):
...
class MqttConfig(ArcherBaseModel):
    enabled: bool = Field(title="Enable MQTT Communication.", default=True)
    host: str = Field(default="", title="MQTT Host")
    port: int = Field(default=1883, title="MQTT Port")
    topic_prefix: str = Field(default="Archer", title="MQTT Topic Prefix")
    client_id: str = Field(default="Archer", title="MQTT Client ID")
...
class PtzAutotrackConfig(ArcherBaseModel):
    enabled: bool = Field(default=False, title="Enable PTZ object autotracking.")
    calibrate_on_startup: bool = Field(
        default=False, title="Perform a camera calibration when Archer starts."
    )
...
class OnvifConfig(ArcherBaseModel):
...
class RetainConfig(ArcherBaseModel):
...
class EventsConfig(ArcherBaseModel):
...
class RecordRetainConfig(ArcherBaseModel):
    days: float = Field(default=0, title="Default retention period.")
    mode: RetainModeEnum = Field(default=RetainModeEnum.all, title="Retain mode.")


class RecordExportConfig(ArcherBaseModel):
    timelapse_args: str = Field(
        default=DEFAULT_TIME_LAPSE_FFMPEG_ARGS, title="Timelapse Args"
    )


class RecordConfig(ArcherBaseModel):
...
class MotionConfig(ArcherBaseModel):
    threshold: int = Field(
        default=30,
...
class StationaryMaxFramesConfig(ArcherBaseModel):
...
class StationaryConfig(ArcherBaseModel):
...
class DetectConfig(ArcherBaseModel):
...
class FilterConfig(ArcherBaseModel):
    min_area: int = Field(
...
class AudioFilterConfig(ArcherBaseModel):
    threshold: float = Field(
        default=0.8,
...
class BirdseyeConfig(ArcherBaseModel):
...
# https://github.com/blakeblackshear/Archer/issues/5659
...
class FfmpegOutputArgsConfig(ArcherBaseModel):
...
class FfmpegConfig(ArcherBaseModel):
...
class CameraInput(ArcherBaseModel):
...
class SnapshotsConfig(ArcherBaseModel):
...
class ColorConfig(ArcherBaseModel):
...
class TimestampStyleConfig(ArcherBaseModel):
...
class CameraMqttConfig(ArcherBaseModel):
...
class RtmpConfig(ArcherBaseModel):
    enabled: bool = Field(default=False, title="RTMP restreaming enabled.")


class CameraLiveConfig(ArcherBaseModel):
...
class CameraUiConfig(ArcherBaseModel):
    order: int = Field(default=0, title="Order of camera in UI.")
    dashboard: bool = Field(
        default=True, title="Show this camera in Archer dashboard UI."
    )


class CameraConfig(ArcherBaseModel):
...
class DatabaseConfig(ArcherBaseModel):
...
class LoggerConfig(ArcherBaseModel):
...
def verify_valid_live_stream_name(
    Archer_config: ArcherConfig, camera_config: CameraConfig
) -> ValueError | None:
    """Verify that a restream exists to use for live view."""
    if (
        camera_config.live.stream_name
        not in Archer_config.go2rtc.dict().get("streams", {}).keys()
    ):
        return ValueError(
...
class ArcherConfig(ArcherBaseModel):
    mqtt: MqttConfig = Field(title="MQTT Configuration.")
    database: DatabaseConfig = Field(
        default_factory=DatabaseConfig, title="Database configuration."
    )
    environment_vars: Dict[str, str] = Field(
        default_factory=dict, title="Archer environment variables."
    )
    ...
    def runtime_config(self, plus_api: PlusApi = None) -> ArcherConfig:
        """Merge camera config with globals."""
        config = self.copy(deep=True)

        # MQTT user/password substitutions
        if config.mqtt.user or config.mqtt.password:
            config.mqtt.user = config.mqtt.user.format(**Archer_ENV_VARS)
            config.mqtt.password = config.mqtt.password.format(**Archer_ENV_VARS)
        ...
        for name, camera in config.cameras.items():
            ...
            for input in camera_config.ffmpeg.inputs:
                input.path = input.path.format(**Archer_ENV_VARS)
