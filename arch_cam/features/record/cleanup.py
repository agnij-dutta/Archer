...
from features.config import ArcherConfig, RetainModeEnum
from features.const import CACHE_DIR, RECORD_DIR
from features.models import Event, Recordings, RecordingsToDelete
from features.record.util import remove_empty_directories
...
class RecordingCleanup(threading.Thread):
    ...
    def __init__(self, config: ArcherConfig, stop_event: MpEvent) -> None:
    ...
    def clean_tmp_clips(self) -> None:
        ...
        for p in Path(CACHE_DIR).rglob("clip_*.mp4"):
            ...
            if p.stat().st_mtime < (datetime.datetime.now().timestamp() - 60 * 1):
                ...
                # empty contents of file before unlinking https://github.com/blakeblackshear/Archer/issues/4769
                with open(p, "w"):
                    pass
