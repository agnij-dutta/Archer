...
from features.comms.dispatcher import Dispatcher
from features.config import ArcherConfig
from features.const import CACHE_DIR, CLIPS_DIR, DRIVER_AMD, DRIVER_ENV_VAR, RECORD_DIR
from features.object_detection import ObjectDetectProcess
from features.types import CameraMetricsTypes, StatsTrackingTypes
from features.util.services import (
    get_amd_gpu_stats,
    get_bandwidth_stats,
    get_cpu_stats,
    get_intel_gpu_stats,
...
def get_latest_version(config: ArcherConfig) -> str:
    if not config.telemetry.version_check:
        return "disabled"

    try:
        request = requests.get(
            "https://api.github.com/repos/blakeblackshear/Archer/releases/latest",
            timeout=10,
        )
...
def stats_init(
    ...
    stats_tracking: StatsTrackingTypes = {
        ...
        "latest_Archer_version": get_latest_version(config),
...
def get_processing_stats(
    config: ArcherConfig, stats: dict[str, str], hwaccel_errors: list[str]
) -> None:
...
async def set_bandwidth_stats(config: ArcherConfig, all_stats: dict[str, Any]) -> None:
...
async def set_gpu_stats(
    config: ArcherConfig, all_stats: dict[str, Any], hwaccel_errors: list[str]
) -> None:
...
def stats_snapshot(
    config: ArcherConfig, stats_tracking: StatsTrackingTypes, hwaccel_errors: list[str]
) -> dict[str, Any]:
    ...
    stats["service"] = {
        ...
        "version": VERSION,
        "latest_version": stats_tracking["latest_Archer_version"],
        "storage": {},
...
class StatsEmitter(threading.Thread):
    def __init__(
        ...
        self.name = "Archer_stats_emitter"
        self.config = config
