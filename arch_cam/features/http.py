...
from features.config import ArcherConfig
...
from features.events.external import ExternalEventProcessor
from features.models import Event, Recordings, Timeline
from features.object_processing import TrackedObject
from features.plus import PlusApi
from features.ptz.onvif import OnvifController
from features.record.export import PlaybackFactorEnum, RecordingExporter
from features.stats import stats_snapshot
from features.storage import StorageMaintainer
from features.util.builtin import (
    clean_camera_user_pass,
    get_tz_modifiers,
    update_yaml_from_url,
)
from features.util.services import ffprobe_stream, restart_Archer, vainfo_hwaccel
...
def create_app(
    ...
    app.Archer_config = Archer_config
...
@bp.route("/")
def is_healthy():
    return "Archer is running. Alive and healthy!"
...
def false_positive(id):
    ...
    if event.false_positive:
        message = "False positive already submitted to Archer+"
        logger.error(message)
        return make_response(jsonify({"success": False, "message": message}), 400)
...
def create_event(camera_name, label):
    if not camera_name or not current_app.Archer_config.cameras.get(camera_name):
        return make_response(
            jsonify(
...
@bp.route("/config")
def config():
    config = current_app.Archer_config.dict()

    for camera_name, camera in current_app.Archer_config.cameras.items():
    ...
    for detector, detector_config in config["detectors"].items():
        detector_config["model"][
            "labelmap"
        ] = current_app.Archer_config.model.merged_labelmap
...
def config_save():
    ...
    try:
        ArcherConfig.parse_raw(new_config)
    except Exception:
        return make_response(
            jsonify(
                {
    ...
    try:
        ...
        return make_response(
            jsonify(
                {
                    "success": False,
                    "message": "Could not write config file, be sure that Archer has write permission on the config file.",
                }
            ),
            400,
        )
    ...
    if save_option == "restart":
        try:
            ...
            logging.error(f"Error restarting Archer: {e}")
            return make_response(
                jsonify(
                    {
                        "success": True,
                        "message": "Config successfully saved, unable to restart Archer",
                    }
                ),
                200,
            )
...
def config_set():
    ...
    try:
        ...
        # Validate the config schema
        try:
            ArcherConfig.parse_raw(new_raw_config)
        except Exception:
            with open(config_file, "w") as f:
...
def config_schema():
    return current_app.response_class(
        current_app.Archer_config.schema_json(), mimetype="application/json"
    )
...
def mjpeg_feed(camera_name):
    ...
    if camera_name in current_app.Archer_config.cameras:
...
def latest_frame(camera_name):
    ...
    if camera_name in current_app.Archer_config.cameras:
        ...
        retry_interval = float(
            current_app.Archer_config.cameras.get(camera_name).ffmpeg.retry_interval
            or 10
        )
        ...
        if frame is None or datetime.now().timestamp() > (
            ...
            if current_app.camera_error_image is None:
                error_image = glob.glob("/opt/Archer/Archer/images/camera-error.jpg")
        ...
        response.headers["Cache-Control"] = "no-store"
        return response
    elif camera_name == "birdseye" and current_app.Archer_config.birdseye.restream:
        frame = cv2.cvtColor(
...
def get_snapshot_from_recording(camera_name: str, frame_time: str):
    if camera_name not in current_app.Archer_config.cameras:
...
def ffprobe():
    ...
    if path_param.startswith("camera"):
        ...
        if camera not in current_app.Archer_config.cameras.keys():
            return make_response(
                jsonify(
        ...
        if not current_app.Archer_config.cameras[camera].enabled:
            return make_response(
        ...
        paths = map(
            ...
            current_app.Archer_config.cameras[camera].ffmpeg.inputs,
        )
...
def logs(service: str):
    log_locations = {
        "Archer": "/dev/shm/logs/Archer/current",
