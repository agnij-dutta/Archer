...
from features.config import CameraConfig, ArcherConfig
...
from features.object_detection import load_labels
from features.types import FeatureMetricsTypes
from features.util.builtin import get_ffmpeg_arg_list
from features.util.services import listen
from features.video import start_or_restart_ffmpeg, stop_ffmpeg
...
def listen_to_audio(
    ...
    setproctitle("Archer.audio_manager")
    listen()
...
class AudioEventMaintainer(threading.Thread):
    ...
    def handle_detection(self, label: str, score: float) -> None:
        if self.detections.get(label):
            ...
            resp = requests.post(
                f"{Archer_LOCALHOST}/api/events/{self.config.name}/{label}/create",
    ...
    def expire_detections(self) -> None:
        ...
        for detection in self.detections.values():
            ...
            if (
                ...
                resp = requests.put(
                    f"{Archer_LOCALHOST}/api/events/{detection['id']}/end",
                    json={
